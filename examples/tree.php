...
class tree extends iup\core {
    ...
    public $dlg, $tree, $vbox, $frame, $db;

    function __construct() {
        try {
            $this->db = new \PDO('sqlite://home/ghost/database/stock.sqlite');
    ...
    function init_tree_atributes() {
        ...
        $this->setAttribute($tree, "TITLE", "DATABASE");
        $this->setAttribute($tree, "ADDBRANCH", "DB");
        $sql = $this->db->query("select name from sqlite_master where type='table'");
        foreach ($sql as $v) {
            $this->setAttribute($tree, "ADDBRANCH1", $v['name']);
        }
